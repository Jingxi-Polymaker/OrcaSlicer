...
- **Flow:** `OrcaSlicer Client` $\rightarrow$ Orca API Gateway(`Cloudflare Worker)` $\rightarrow$ `Supabase DB`
...
- **Responsibility 1: API Gateway Validation.** Validates the JWT signature using ES256 (ECDSA P-256) via Supabase's JWKS endpoint. Invalid requests are rejected at the API Gateway (e.g., in Singapore or London) before they ever reach the database.
