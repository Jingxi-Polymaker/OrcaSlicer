...
private:
    ...
    bool fetch_slot_info(const std::string& host,
                         std::vector<QidiSlotInfo>& slots,
                         int& box_count,
                         std::string& error) const;
    bool fetch_filament_dict(const std::string& host,
                             QidiFilamentDict& dict,
                             std::string& error) const;

    static void parse_ini_section(const std::string& content,
                                  const std::string& section_name,
                                  std::map<int, std::string>& result);
    static void parse_filament_sections(const std::string& content,
                                        std::map<int, std::string>& result);
