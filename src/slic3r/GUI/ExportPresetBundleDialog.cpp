...
namespace Slic3r { 
    namespace GUI {
        ...
        ExportPresetBundleDialog::ExportPresetBundleDialog(wxWindow *parent, wxWindowID id, const wxString &title, const wxPoint &pos, const wxSize &size, long style)
        ...
        {
            wxString TargetUrl = from_u8( (boost::filesystem::path(resources_dir()) / "web/dialog/ExportPresetDialog/index.html").make_preferred().string() );
            ...
            try {
                ...
                else if( strCmd == "export_local")
                {
                    wxFileDialog dlg(this,
                                     _L("Save preset bundle"),
                                     "",
                                     "export.orca_bundle",
                                     "Orca Preset Bundle (*.orca_bundle)|*.orca_bundle",
        ...
        void ExportPresetBundleDialog::InitExportData()
        {
            ...
            for (const Preset &printer_preset : printer_presets) {
                ...
                if (preset_bundle.printers.select_preset_by_name(preset_name, true)) {
                    ...
                    {
                        boost::filesystem::path file_path(data_dir() + "/" + PRESET_USER_DIR + "/" + "Temp" + "/" + (new_printer_preset->name + ".json"));
        ...
        void ExportPresetBundleDialog::OnExportData(const wxString &path,const wxString& filename, json data)
        {
            ...
            for (const auto& name : filament_names) {
                ...
                for (const auto& vp : it->second) {
                    if (vp.second) selected_filaments.push_back(vp.second);
                }
            }
            ...
            if (export_path.empty() || "initial_failed" == export_path) return;
